class AnonRoutesController < ApplicationController
  ...
  def create
    ...
    new_route.map_url = "https://www.google.com/maps?saddr=#{start.latitude},+#{start.longitude}&daddr=#{wp1.latitude},+#{wp1.longitude}+to:#{wp2.latitude},+#{wp2.longitude}+to:#{wp3.latitude},+#{wp3.longitude}+to:#{start.latitude},+#{start.longitude}&dirflg=w"
    new_route.save
